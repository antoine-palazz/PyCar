...
def clean_df(dataframe):
    ...
    droping_liste = list(
        set(
            df_bornes[df_bornes["xlongitude"].isna()].index.to_list()
            + df_bornes[df_bornes["ylatitude"].isna()].index.to_list()
        )
    )
    ...
    for k in range(len(Trajet_thermique_lat) - 1):
        if parcourue <= autonomie * 0.7:
            ...
            station = recherche_station_proche(
                df_bornes,
                autonomie * 0.3,
                Trajet_thermique_lat[k],
                Trajet_thermique_lon[k],
            )
            if station == None:
                return None
            else:
...
class Autoroute(CarNetwork):
    ...
    def clean_base(self):
        ...
        self.stations_peages["x"] = (
            self.stations_peages["x"].str.replace(",", ".").astype(float)
        )
        self.stations_peages["y"] = (
            self.stations_peages["y"].str.replace(",", ".").astype(float)
        )
        ...
        self.stations_peages["xlongitude"] = lambert93_to_latlon(
            self.stations_peages["x"], self.stations_peages["y"]
        )[0]
        self.stations_peages["ylatitude"] = lambert93_to_latlon(
            self.stations_peages["x"], self.stations_peages["y"]
        )[1]

        ## On renomme les colonnes par soucis de clarté
        self.stations_peages.rename(
            columns={"x": "lambert93_x", "y": "lambert93_y"}, inplace=True
        )
    ...
    def plot_peages_autoroutes(self, map):
        ...
        for index, lat, lon in df[["ylatitude", "xlongitude"]].itertuples():
            ...
            folium.RegularPolygonMarker(
                location=[lon, lat], color="blue", radius=5
            ).add_to(map)
...
def cout_trajet_electrique(
    start, autonomie_véhicule, autonomie_start, dist, liste_localisation_bornes, prix
):
    """
    Parameters:
    -----------
    ...
    for i, sous_liste in enumerate(liste_localisation_bornes):
        ...
        if i == 0:
            distance = distance_entre_2_bornes(
                start, sous_liste[0:2]
            )  # distance entre le point de départ et la borne 1
        else:  # i > 0
            sous_liste_avant = liste_localisation_bornes[i - 1]
            borne_avant = sous_liste_avant[0:2]  # [lat, lon]
            distance = distance_entre_2_bornes(
                borne_avant, sous_liste[0:2]
            )  # distance entre la borne i-1 et i
        autonomie_restante = autonomie_start - distance
        if autonomie_restante <= 0:
            print(
                f"Le trajet trouvé n'est pas le bon, le véhicule tombe en panne avant d'avoir pu atteindre la borne {i}"
            )
        else:
            nb_km_remplir = (
                autonomie_véhicule - autonomie_restante
            )  # nb de km qu'il faut pour que le véhicule ait une autonomie max
            prix_100km = (
                20 * prix
            )  # on suppose que le véhicule roule sur une autoroute (donc 20kwH pour 100km)
            cout_auto_max = (
                nb_km_remplir * prix_100km
            ) / 100  # prix pour que le véhicule soit à son autonomie maximale
