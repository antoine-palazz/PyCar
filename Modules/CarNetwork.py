...
class CarNetwork:
    ...
    def clean_data(self):
        ...
        droping_liste = list(
            set(
                self.stations_data[
                    self.stations_data["xlongitude"].isna()
                ].index.to_list()
                + self.stations_data[
                    self.stations_data["ylatitude"].isna()
                ].index.to_list()
            )
        )
        ...
        def transform_acces(row):
            if not pd.isna(row):  # On ne peut rien dire des nan
                ...
                if "payant" in mots:
                    row = "payant"
                elif "gratuit" in mots:
                    row = "gratuit"
                for mot in mots:
                    if len(mot.split("€")) > 1:
                        row = "payant"
                    if mot == "carte" or mot == "badge":
                        row = "carte ou badge"
                    if mot == "oui":
                        row = "information manquante"
                # else: row = 'accès spécial'
            else:
                row = "information manquante"
            return row

        self.stations_data["acces_recharge"] = self.stations_data[
            "acces_recharge"
        ].apply(transform_acces)
    ...
    def get_coordo(self):
        ...
        """
        dep_json_A = requests.get(
            "https://api-adresse.data.gouv.fr/search/?q="
            + urllib.parse.quote(self.A)
            + "&format=json"
        ).json()
        dep_json_B = requests.get(
            "https://api-adresse.data.gouv.fr/search/?q="
            + urllib.parse.quote(self.B)
            + "&format=json"
        ).json()
    ...
    def get_route_map(self):
        ...
        folium.plugins.AntPath(
            locations=trajet, reverse="True", dash_array=[20, 30]
        ).add_to(carte)
    ...
    def distance_via_routes(self):
        ...
        for i in range(len(trajet) - 1):
            ...
            if self.autonomie < distance_1:
                ...
                j = (
                    j + 1
                )  # compte combien de fois l'autonomie a été saturée pour pénaliser
    ...
    def nearest_stations(self, stop_coord, distance_max):
        ...
        stations = stations[
            (stations["ylatitude"] >= -90)
            & (stations["ylatitude"] <= 90)
            & (stations["xlongitude"] >= -90)
            & (stations["xlongitude"] <= 90)
        ]
        ...
        is_in_range = (
            lambda location, coord, distance: geodesic(location, coord).kilometers
            <= distance
        )
        ...
        for i in range(len(stop_coord)):
            ...
            location_tuples = [
                list(element)
                for element in loc_tuples
                if is_in_range(location, element, distance_max)
            ]
    ...
    def plot_stations(self, map):
        ...
        df2.rename(
            columns={"xlongitude": "Longitude", "ylatitude": "Latitude"}, inplace=True
        )
        ...
        def bornes_dans_region(df1, df2):
            ...
            df3 = df2[
                [
                    point_inside_polygon(lat, lon, region_poly)
        ...
        for index, lat, lon, com, acces_recharge in df[
            ["ylatitude", "xlongitude", "n_station", "acces_recharge"]
        ].itertuples():
            # Créez un marqueur avec une couleur différente en fonction des valeurs
            if acces_recharge == "payant":
                fill_color = "red"
            elif acces_recharge == "gratuit":
                fill_color = "green"
            elif acces_recharge == "information manquante":
                fill_color = "grey"
            elif acces_recharge == "carte ou badge":
                fill_color = "cyan"
            elif acces_recharge == "charges gratuites de 12 à 14h et de 19h à 21h":
                fill_color = "yellow"
    ...
    def plot_accidents(self, map, path):
        ...
        accidents_2022idf_carac["Latitude"] = (
            accidents_2022idf_carac["Latitude"].str.replace(",", ".").astype(float)
        )

        # Remplace les virgules par des points et convertit la colonne "Longitude" en flottants
        accidents_2022idf_carac["Longitude"] = (
            accidents_2022idf_carac["Longitude"].str.replace(",", ".").astype(float)
        )

        # on trie les données d'accident en fonction de leur localisation
        dict_accidents_2022idf = accidents_2022idf_carac.groupby(
            ["Commune", "Adresse"]
        ).groups
        # on trie le dictionnaire obtenu
        dict_trie_accidents_2022idf = dict(
            sorted(
                dict_accidents_2022idf.items(),
        ...
        for key in dict_trie_accidents_2022idf.keys():
            ...
            coord_ligne1 = [
                accidents_2022idf_carac.iloc[dict_trie_accidents_2022idf[key][m1]][
                    "Longitude"
                ],
                accidents_2022idf_carac.iloc[dict_trie_accidents_2022idf[key][m1]][
                    "Latitude"
            ...
            coord_ligne2 = [
                accidents_2022idf_carac.iloc[dict_trie_accidents_2022idf[key][m2]][
                    "Longitude"
                ],
                accidents_2022idf_carac.iloc[dict_trie_accidents_2022idf[key][m2]][
                    "Latitude"
            ...
            folium.PolyLine(
                [coord_ligne1, coord_ligne2],
                popup=key,
                weight=0.7
                * len(
                    dict_trie_accidents_2022idf[key]
                    / len(
                        dict_trie_accidents_2022idf[
                            ("93066 - Saint-Denis", "AUTOROUTE A1")
                        ]
                    )
                ),
            ).add_to(map)
    ...
    def cout_distance_thermique(self, prix_essence, essence=True):
        ...
        if essence == True:
            conso_100k = (
                7.54  # nombre de litre consommé par le véhicule à essence sur 100km
            )

        else:
            conso_100k = (
                6.11  # nombre de litre consommé par le véhicule disesel sur 100km
            )

        nb_litre_trajet = (
            distance * conso_100k
        ) / 100  # nombre de litre consommé par le véhicule sur la distance dist
