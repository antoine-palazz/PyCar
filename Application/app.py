...
def page_suivante1():
    def graph_intro():
        ...
        plt.plot(
            df["Date"],
            df["Nombre"],
            label="Évolution du parc de véhicules électriques en France",
            color="blue",
            marker="o",
        )
        plt.xlabel("Date")
        ...
        plt.xticks(
            rotation=45
        )  # Rotation des étiquettes de l'axe des x pour rendre les dates plus lisibles
        plt.legend()
        plt.fill_between(
            df["Date"], df["Nombre"], color="lightgray", alpha=0.7
        )  # Fond gris clair
